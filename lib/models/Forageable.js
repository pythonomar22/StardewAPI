...
module.exports = class Forageable {
  ...
  static async updateById(id, { name, abt, where_tofind, img }) {
    ...
    const { rows } = await pool.query(
      'UPDATE forageables SET name=$2, abt=$3, where_tofind=ARRAY [$4], img=$5 WHERE id=$1 RETURNING *',
      [id, name, abt, where_tofind, img]
    )
