...
module.exports = class Artifact {
  ...
  static async insert({  name, abt, sell_price, img }) {
    const rows = this.getRows('INSERT INTO artifacts(name, abt, sell_price, img) VALUES ($1, $2, $3, $4) RETURNING *',  [name, abt, sell_price, img]  )
  ...
  static async getById(id) {
    const rows = this.getRows('SELECT * FROM artifacts WHERE id=$1', [id])
  ...
  static async getRows(query, fields) {
